...
**Насколько вообще нужен space, если у нас просто отдельный iFrame с уроком? Создание space это опционально и его может и не быть? Или space есть всегда?**

Space всегда есть. К space можно относиться просто как к директории для комнат. Можно создать их несколько, а можно все комнаты хранить в одном space. Это родительский компонент для комнаты и всех имеющихся сущностей в конференции.

**В ссылке-приглашении на комнату `https://edu.livedigital.space/room/0mrw9W62rC` не указан space, значит space это опцинально?**

В ссылке используется alias, это сделано просто для сокращения длины ссылки, так как alias сам по себе уникален.

**Могу ли я сделать запрос без указания space? Чтобы создателю было достаточно лишь указать просто ссылку на комнату**
...
**Нам нужна возможность в начале сессии передать емейл пользователя, чтобы в конце сессии получить данные о посещаемости. В идеале нам надо каким-то методом отправлять запрос и получать список посещений, в виде адреса емейл и времени, проведённого на занятии, а также общее время встречи.**
...
Да, в случае ошибки лучше начать заново, мы автоматом этого не делаем.
